...
def upload_package(name, version, content, url, jsprogram):
    ...
    with pool.connect() as db_conn:
        ...
        print(f'hash: {contentHash} content: {type(content)}, {len(content)}')
        db_conn.execute(insert_stmt, parameters={"ID": id, "Name": name, "Version": version, "Content": content[4:], "URL": url, "JSProgram": jsprogram, "ContentHash": contentHash})
